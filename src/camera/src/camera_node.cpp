/*
 * This is a simplified version of the ROS node for handling cameras
 * It is currently designed for testing, not performance
 * Needs to be thread safe
 * Scaling and dimensions need to be fixed
 * Needs to be refactored, code is gross
 */
#include <cstdio>
#include <fstream>
#include <iostream>
  ...
  public:
    ...
    void post_process(Mat &input_image, vector<Mat> &outputs, const vector<string> &class_name, bool front) {
      ...
      for (size_t i = 0; i < outputs.size(); ++i) {
        ...
        // ones with confidence scores over the threshold. Assign the box's class label as the class
        ...
        for (int j = 0; j < outputs[i].rows; ++j, data += outputs[i].cols) {
          ...
          if (confidence > confThreshold) {
            // these dimensions shouldn't be hardcoded
    ...
    void detect_front() {
      ...
      while (!this->front_detected) {
        ...
        if (this->front_frame.empty()) break;
    ...
    void detect_bottom() {
      ...
      while (!this->bottom_detected) {
        ...
        if (this->bottom_frame.empty()) break;
        vector<Mat> detections;
